...
switch (true) {
	case $act=="list":
		$items = getItems(array("table" => $table, "condition" => "where product_id like '{$product['id']}'", "limit" => 20, "pagination" => true));
		$paging = $pagination->getSource("&p");
		$template = "product-tab/list";
		break;
	case $act=="edit":
		if($_REQUEST['id'] != "") {
		...
		$template = "product-tab/edit";
		break;
	case $act=="save":
		if(isset($_POST['savebtn'])) {
			$data = array("title", "thumbnail", "product_id", "enable", "index");
