...
if(is_array(@$category)) {
	$item = getTranslate("category", $row_language[0]['id'], $category['id']);
...
switch (true) {
	case $act=="list":
		$items = getItems(array("table" => $table, "condition" => "where category_id like '{$category['id']}'", "limit" => 20, "pagination" => true));
		$paging = $pagination->getSource("&p");
		$template = "category-tab/list";
		break;
	case $act=="edit":
		if($_REQUEST['id'] != "") {
		...
		$template = "category-tab/edit";
		break;
	case $act=="save":
		if(isset($_POST['savebtn'])) {
			$data = array("title", "category_id", "enable", "index");
