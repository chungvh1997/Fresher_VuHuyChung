<div class="well">Thêm / Cập nhật thẻ <?= @$item['title']!="" ? "'".$item['title']."' thuộc " : "thuộc "?>sản phẩm '<?=!empty($product) ? implode(" ", array_slice(explode(" ", $product['title']), 0, 3)) . (substr_count($product['title'], " ")>2 ? "..." : "") : ""?>'
</div>
...
<form action="<?=preg_replace('/(&id=\d*)/', '', str_replace("&act=edit", "&act=save", getCurrentPageURL()))?>" method="post" enctype="multipart/form-data" accept-charset="UTF-8">
	<table class="table">
		...
		<tr>
			<td class="col-xs-2"><label class="form-control">Hình icon hiện tại:</label></td>
			<td class="col-xs-10">
				<img id="thumb" src="<?=$item['thumbnail']!=''?(count(explode("://", $item['thumbnail']))>1?$item['thumbnail']:str_replace("//", "/", $item['thumbnail'])):''?>" alt="Chưa có hình icon đại diện" onclick="openBrowser('#thumb', '#thumbnail');" style="cursor: pointer;">&nbsp;&nbsp;&nbsp;
				<button type="button" class="btn btn-primary" onclick="openBrowser('#thumb', '#thumbnail');">Chọn hình icon</button>&nbsp;&nbsp;&nbsp;
				<button type="button" class="btn btn-danger" onclick="$('#thumbnail').val('');$('#thumb').attr('src','');">Xóa hình icon hiện tại</button>
